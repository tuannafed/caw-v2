...
- **`lower-case.md`** — **fill-in**: a file the project *writes into*. Two flavors:
  ...
  - *Project seed prose* (`knowledge.md`, `harness-backlog.md`) —
    scaffolded near-empty, then filled and appended to over the project's life.
    (Project conventions/rules are NOT here — they live in `.claude/rules/project.md`,
    the single auto-injected source of truth generated by `/caw:setup`.)
...
project/
  ...
  docs/
    caw/              # caw policy + prose the agents read/write
      ...
      knowledge.md  harness-backlog.md                      # project seed prose
