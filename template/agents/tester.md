...
- `javascript-testing-patterns` — generic Jest / Vitest / Testing Library setup
- `webapp-testing` — Anthropic Playwright E2E patterns (only when writing E2E)
- **For React component tests in jsdom: ALSO `react-component-testing`** — leak-free QueryClient + RTL patterns. Without this skill the agent will sink into the QueryClient-per-render anti-pattern that hangs jest.
- For mobile tasks (apps/mobile/, react-native, expo): also `react-native-best-practices`
...
If you must write a component test, follow the `react-component-testing` skill exactly (hoisted QueryClient, `gcTime: 0`, `userEvent.setup({ delay: null })`, mocked mutations invoke `onSuccess`, max 5-7 `it()` per file).
...
- **Load every required testing skill before writing tests** (`rules/common/skill-loading.md`).
...
- **Default to SKIP component tests for frontend tasks.** Write hook/util/store/schema tests instead — they're 10x faster and catch the same bugs. Component tests only when explicitly mandated AND when following `react-component-testing` skill patterns (hoisted QueryClient, no factory-per-render, `userEvent.setup({ delay: null })`).
